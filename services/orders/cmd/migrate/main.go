...
import (
	...
	"github.com/MamangRust/simple_microservice_ecommerce/order/pkg/otel"
...
func main() {
	...
	telemetry := otel.NewTelemetry(otel.Config{
		ServiceName:    "order-migrate-service",
	...
	logger, err := logger.NewLogger("order-migrate-service", telemetry.GetLogger())
	...
	connStr := fmt.Sprintf("host=%s port=%s user=%s dbname=%s password=%s sslmode=disable",
		viper.GetString("DB_ORDER_HOST"),
		viper.GetString("DB_ORDER_PORT"),
		viper.GetString("DB_ORDER_USERNAME"),
		viper.GetString("DB_ORDER_NAME"),
		viper.GetString("DB_ORDER_PASSWORD"),
	)
