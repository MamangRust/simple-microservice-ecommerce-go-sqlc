...
func NewServer(ctx context.Context) (*Server, func(context.Context) error, error) {
	...
	logger, err := logger.NewLogger("order-service", telemetry.GetLogger())
	if err != nil {
	...
	myredis := redis.NewClient(&redis.Options{
		Addr:         fmt.Sprintf("%s:%s", viper.GetString("REDIS_HOST_ORDER"), viper.GetString("REDIS_PORT_ORDER")),
		Password:     viper.GetString("REDIS_PASSWORD_ORDER"),
