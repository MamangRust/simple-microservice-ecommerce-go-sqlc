...
func NewClient(logger logger.LoggerInterface) (*sql.DB, error) {
	dbDriver := viper.GetString("DB_USER_DRIVER")
	if dbDriver == "" {
	...
	switch dbDriver {
	case "postgres":
		host := viper.GetString("DB_USER_HOST")
		port := viper.GetString("DB_USER_PORT")
		user := viper.GetString("DB_USER_USERNAME")
		dbname := viper.GetString("DB_USER_NAME")
		password := viper.GetString("DB_USER_PASSWORD")
