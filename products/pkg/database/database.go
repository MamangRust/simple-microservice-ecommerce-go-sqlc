...
func NewClient(logger logger.LoggerInterface) (*sql.DB, error) {
	dbDriver := viper.GetString("DB_PRODUCT_DRIVER")
	if dbDriver == "" {
	...
	switch dbDriver {
	case "postgres":
		host := viper.GetString("DB_PRODUCT_HOST")
		port := viper.GetString("DB_PRODUCT_PORT")
		user := viper.GetString("DB_PRODUCT_USERNAME")
		dbname := viper.GetString("DB_PRODUCT_NAME")
		password := viper.GetString("DB_PRODUCT_PASSWORD")
	...
	if maxOpenConns == 0 {
		maxOpenConns = 25
		logger.Debug("Using default value for DB_MAX_OPEN_CONNS", zap.Int("default", maxOpenConns))
	}
