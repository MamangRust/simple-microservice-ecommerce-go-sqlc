...
func NewClient(logger logger.LoggerInterface) (*sql.DB, error) {
	dbDriver := viper.GetString("DB_ROLE_DRIVER")
	if dbDriver == "" {
	...
	switch dbDriver {
	case "postgres":
		host := viper.GetString("DB_ROLE_HOST")
		port := viper.GetString("DB_ROLE_PORT")
		user := viper.GetString("DB_ROLE_USERNAME")
		dbname := viper.GetString("DB_ROLE_NAME")
		password := viper.GetString("DB_ROLE_PASSWORD")
