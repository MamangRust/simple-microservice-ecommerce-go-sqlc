...
func NewClient(logger logger.LoggerInterface) (*sql.DB, error) {
	dbDriver := viper.GetString("DB_AUTH_DRIVER")
	if dbDriver == "" {
	...
	switch dbDriver {
	case "postgres":
		host := viper.GetString("DB_AUTH_HOST")
		port := viper.GetString("DB_AUTH_PORT")
		user := viper.GetString("DB_AUTH_USERNAME")
		dbname := viper.GetString("DB_AUTH_NAME")
		password := viper.GetString("DB_AUTH_PASSWORD")
