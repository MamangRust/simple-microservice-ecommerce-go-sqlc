...
func NewRoleHandleApi(router *echo.Echo, queryClient pb.RoleQueryServiceClient,
	commandClient pb.RoleCommandServiceClient, logger logger.LoggerInterface, grpcmiddleware middlewares.GRPCErrorHandlingMiddleware) *roleHandleApi {
...
func (h *roleHandleApi) FindAll(c echo.Context) error {
	...
	h.logger.Info("Attempting to find all roles",
	...
	if err != nil {
		h.logger.Error("Failed to find all roles via query client", zap.Error(err))
		return errors.ErrApiFailedFindAll(c)
	}

	so := h.mapping.ToApiResponsePaginationRole(res)
	h.logger.Info("Successfully found all roles", zap.String("handler", "FindAll"))
...
func (h *roleHandleApi) FindById(c echo.Context) error {
	...
	if err != nil || roleID <= 0 {
		h.logger.Warn("Invalid role ID provided in FindById", zap.String("handler", "FindById"), zap.String("param_id", c.Param("id")))
		return errors.ErrApiRoleInvalidId(c)
	}

	h.logger.Info("Attempting to find role by ID", zap.String("handler", "FindById"), zap.Int("role_id", roleID))
	...
	if err != nil {
		h.logger.Error("Failed to find role by ID via query client", zap.Int("role_id", roleID), zap.Error(err))
		return errors.ErrApiRoleNotFound(c)
	}

	so := h.mapping.ToApiResponseRole(res)
	h.logger.Info("Successfully found role by ID", zap.String("handler", "FindById"), zap.Int("role_id", roleID))
...
func (h *roleHandleApi) FindByActive(c echo.Context) error {
	...
	h.logger.Info("Attempting to find active roles",
	...
	if err != nil {
		h.logger.Error("Failed to find active roles via query client", zap.Error(err))
		return errors.ErrApiFailedFindActive(c)
	}

	so := h.mapping.ToApiResponsePaginationRoleDeleteAt(res)
	h.logger.Info("Successfully found active roles", zap.String("handler", "FindByActive"))
...
func (h *roleHandleApi) FindByTrashed(c echo.Context) error {
	...
	h.logger.Info("Attempting to find trashed roles",
	...
	if err != nil {
		h.logger.Error("Failed to find trashed roles via query client", zap.Error(err))
		return errors.ErrApiFailedFindTrashed(c)
	}

	so := h.mapping.ToApiResponsePaginationRoleDeleteAt(res)
	h.logger.Info("Successfully found trashed roles", zap.String("handler", "FindByTrashed"))
...
func (h *roleHandleApi) FindByUserId(c echo.Context) error {
	...
	if err != nil || userID <= 0 {
		h.logger.Warn("Invalid user ID provided in FindByUserId", zap.String("handler", "FindByUserId"), zap.String("param_user_id", c.Param("user_id")))
		return errors.ErrApiRoleInvalidId(c)
	}

	h.logger.Info("Attempting to find roles by user ID", zap.String("handler", "FindByUserId"), zap.Int("user_id", userID))
	...
	if err != nil {
		h.logger.Error("Failed to find roles by user ID via query client", zap.Int("user_id", userID), zap.Error(err))
		return errors.ErrApiRoleNotFound(c)
	}

	so := h.mapping.ToApiResponsesRole(res)
	h.logger.Info("Successfully found roles by user ID", zap.String("handler", "FindByUserId"), zap.Int("user_id", userID))
...
func (h *roleHandleApi) Create(c echo.Context) error {
	...
	if err := c.Bind(&req); err != nil {
		h.logger.Warn("Failed to bind CreateRole request", zap.Error(err))
		return errors.ErrApiBindCreateRole(c)
	}

	if err := req.Validate(); err != nil {
		h.logger.Warn("Failed to validate CreateRole request", zap.String("name", req.Name), zap.Error(err))
		return errors.ErrApiValidateCreateRole(c)
	}

	h.logger.Info("Attempting to create a new role", zap.String("handler", "Create"), zap.String("name", req.Name))
	...
	if err != nil {
		h.logger.Error("Failed to create role via command client", zap.String("name", req.Name), zap.Error(err))
		return errors.ErrApiFailedCreateRole(c)
	}

	so := h.mapping.ToApiResponseRole(res)
	h.logger.Info("Successfully created new role", zap.String("handler", "Create"), zap.String("name", req.Name))
...
func (h *roleHandleApi) Update(c echo.Context) error {
	...
	roleID, err := strconv.Atoi(c.Param("id"))
	if err != nil || roleID <= 0 {
		h.logger.Warn("Invalid role ID provided in Update", zap.String("handler", "Update"), zap.String("param_id", c.Param("id")))
		return errors.ErrInvalidRoleId(c)
	}

	var req requests.UpdateRoleRequest
	if err := c.Bind(&req); err != nil {
		h.logger.Warn("Failed to bind UpdateRole request", zap.Error(err))
		return errors.ErrApiBindUpdateRole(c)
	}

	if err := req.Validate(); err != nil {
		h.logger.Warn("Failed to validate UpdateRole request", zap.Int("role_id", roleID), zap.String("name", req.Name), zap.Error(err))
		return errors.ErrApiValidateUpdateRole(c)
	}

	h.logger.Info("Attempting to update role", zap.String("handler", "Update"), zap.Int("role_id", roleID), zap.String("name", req.Name))
	...
	if err != nil {
		h.logger.Error("Failed to update role via command client", zap.Int("role_id", roleID), zap.Error(err))
		return errors.ErrApiFailedUpdateRole(c)
	}

	so := h.mapping.ToApiResponseRole(res)
	h.logger.Info("Successfully updated role", zap.String("handler", "Update"), zap.Int("role_id", roleID))
...
func (h *roleHandleApi) Trashed(c echo.Context) error {
	...
	roleID, err := strconv.Atoi(c.Param("id"))
	if err != nil || roleID <= 0 {
		h.logger.Warn("Invalid role ID provided in Trashed", zap.String("handler", "Trashed"), zap.String("param_id", c.Param("id")))
		return errors.ErrInvalidRoleId(c)
	}

	h.logger.Info("Attempting to trash role", zap.String("handler", "Trashed"), zap.Int("role_id", roleID))

	req := &pb.FindByIdRoleRequest{RoleId: int32(roleID)}
	res, err := h.commandClient.TrashedRole(ctx, req)
	if err != nil {
		h.logger.Error("Failed to trash role via command client", zap.Int("role_id", roleID), zap.Error(err))
		return errors.ErrApiFailedTrashedRole(c)
	}

	so := h.mapping.ToApiResponseRoleDeleteAt(res)
	h.logger.Info("Successfully trashed role", zap.String("handler", "Trashed"), zap.Int("role_id", roleID))
...
func (h *roleHandleApi) Restore(c echo.Context) error {
	...
	roleID, err := strconv.Atoi(c.Param("id"))
	if err != nil || roleID <= 0 {
		h.logger.Warn("Invalid role ID provided in Restore", zap.String("handler", "Restore"), zap.String("param_id", c.Param("id")))
		return errors.ErrInvalidRoleId(c)
	}

	h.logger.Info("Attempting to restore role", zap.String("handler", "Restore"), zap.Int("role_id", roleID))

	req := &pb.FindByIdRoleRequest{RoleId: int32(roleID)}
	res, err := h.commandClient.RestoreRole(ctx, req)
	if err != nil {
		h.logger.Error("Failed to restore role via command client", zap.Int("role_id", roleID), zap.Error(err))
		return errors.ErrApiFailedRestoreRole(c)
	}

	so := h.mapping.ToApiResponseRoleDeleteAt(res)
	h.logger.Info("Successfully restored role", zap.String("handler", "Restore"), zap.Int("role_id", roleID))
...
func (h *roleHandleApi) DeletePermanent(c echo.Context) error {
	...
	roleID, err := strconv.Atoi(c.Param("id"))
	if err != nil || roleID <= 0 {
		h.logger.Warn("Invalid role ID provided in DeletePermanent", zap.String("handler", "DeletePermanent"), zap.String("param_id", c.Param("id")))
		return errors.ErrInvalidRoleId(c)
	}

	h.logger.Info("Attempting to permanently delete role", zap.String("handler", "DeletePermanent"), zap.Int("role_id", roleID))

	req := &pb.FindByIdRoleRequest{RoleId: int32(roleID)}
	res, err := h.commandClient.DeleteRolePermanent(ctx, req)
	if err != nil {
		h.logger.Error("Failed to permanently delete role via command client", zap.Int("role_id", roleID), zap.Error(err))
		return errors.ErrApiFailedDeletePermanent(c)
	}

	so := h.mapping.ToApiResponseRoleDelete(res)
	h.logger.Info("Successfully permanently deleted role", zap.String("handler", "DeletePermanent"), zap.Int("role_id", roleID))
...
func (h *roleHandleApi) RestoreAll(c echo.Context) error {
	...
	h.logger.Info("Attempting to restore all trashed roles", zap.String("handler", "RestoreAll"))

	res, err := h.commandClient.RestoreAllRole(ctx, &emptypb.Empty{})
	if err != nil {
		h.logger.Error("Failed to restore all roles via command client", zap.Error(err))
		return errors.ErrApiFailedRestoreAll(c)
	}

	so := h.mapping.ToApiResponseRoleAll(res)
	h.logger.Info("Successfully restored all trashed roles", zap.String("handler", "RestoreAll"))
...
func (h *roleHandleApi) DeleteAllPermanent(c echo.Context) error {
	...
	h.logger.Info("Attempting to permanently delete all trashed roles", zap.String("handler", "DeleteAllPermanent"))

	res, err := h.commandClient.DeleteAllRolePermanent(ctx, &emptypb.Empty{})
	if err != nil {
		h.logger.Error("Failed to permanently delete all roles via command client", zap.Error(err))
		return errors.ErrApiFailedDeleteAll(c)
	}

	so := h.mapping.ToApiResponseRoleAll(res)
	h.logger.Info("Successfully permanently deleted all trashed roles", zap.String("handler", "DeleteAllPermanent"))
