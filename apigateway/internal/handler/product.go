...
func NewProductHandle(router *echo.Echo, queryClient pb.ProductQueryServiceClient, commandClient pb.ProductCommandServiceClient, logger logger.LoggerInterface, grpcmiddleware middlewares.GRPCErrorHandlingMiddleware) *productHandleApi {
...
func (h *productHandleApi) FindAllProduct(c echo.Context) error {
	...
	h.logger.Info("Attempting to find all products",
	...
	if err != nil {
		h.logger.Error("Failed to find all products via query client", zap.Error(err))
		return errors.ErrApiProductFailedFindAll(c)
	}

	so := h.mapping.ToApiResponsePaginationProduct(res)
	h.logger.Info("Successfully found all products", zap.String("handler", "FindAllProduct"))
...
func (h *productHandleApi) FindById(c echo.Context) error {
	...
	if err != nil {
		h.logger.Warn("Invalid product ID provided in FindById", zap.String("handler", "FindById"), zap.String("param_id", c.Param("id")))
		return errors.ErrApiProductInvalidId(c)
	}

	h.logger.Info("Attempting to find product by ID", zap.String("handler", "FindById"), zap.Int("product_id", id))

	req := &pb.FindByIdProductRequest{Id: int32(id)}
	res, err := h.queryClient.FindById(ctx, req)
	if err != nil {
		h.logger.Error("Failed to find product by ID via query client", zap.Int("product_id", id), zap.Error(err))
		return errors.ErrApiProductFailedFindById(c)
	}

	so := h.mapping.ToApiResponseProduct(res)
	h.logger.Info("Successfully found product by ID", zap.String("handler", "FindById"), zap.Int("product_id", id))
...
func (h *productHandleApi) FindByActive(c echo.Context) error {
	...
	h.logger.Info("Attempting to find active products",
	...
	if err != nil {
		h.logger.Error("Failed to find active products via query client", zap.Error(err))
		return errors.ErrApiProductFailedFindByActive(c)
	}

	so := h.mapping.ToApiResponsePaginationProductDeleteAt(res)
	h.logger.Info("Successfully found active products", zap.String("handler", "FindByActive"))
...
func (h *productHandleApi) FindByTrashed(c echo.Context) error {
	...
	h.logger.Info("Attempting to find trashed products",
	...
	if err != nil {
		h.logger.Error("Failed to find trashed products via query client", zap.Error(err))
		return errors.ErrApiProductFailedFindByTrashed(c)
	}

	so := h.mapping.ToApiResponsePaginationProductDeleteAt(res)
	h.logger.Info("Successfully found trashed products", zap.String("handler", "FindByTrashed"))
...
func (h *productHandleApi) Create(c echo.Context) error {
	...
	if err := c.Bind(&req); err != nil {
		h.logger.Warn("Failed to bind CreateProduct request", zap.Error(err))
		return errors.ErrApiBindCreateProduct(c)
	}

	if err := req.Validate(); err != nil {
		h.logger.Warn("Failed to validate CreateProduct request", zap.String("name", req.Name), zap.Error(err))
		return errors.ErrApiValidateCreateProduct(c)
	}

	h.logger.Info("Attempting to create a new product", zap.String("handler", "Create"), zap.String("name", req.Name), zap.Int64("price", int64(req.Price)), zap.Int("stock", req.Stock))
	...
	if err != nil {
		h.logger.Error("Failed to create product via command client", zap.String("name", req.Name), zap.Error(err))
		return errors.ErrApiProductFailedCreate(c)
	}

	so := h.mapping.ToApiResponseProduct(res)
	h.logger.Info("Successfully created new product", zap.String("handler", "Create"), zap.String("name", req.Name), zap.Int32("new_product_id", int32(so.Data.ID)))
...
func (h *productHandleApi) Update(c echo.Context) error {
	...
	if err != nil {
		h.logger.Warn("Invalid product ID provided in Update", zap.String("handler", "Update"), zap.String("param_id", c.Param("id")))
		return errors.ErrApiProductInvalidId(c)
	}

	var req requests.UpdateProductRequest
	if err := c.Bind(&req); err != nil {
		h.logger.Warn("Failed to bind UpdateProduct request", zap.Error(err))
		return errors.ErrApiBindUpdateProduct(c)
	}

	if err := req.Validate(); err != nil {
		h.logger.Warn("Failed to validate UpdateProduct request", zap.Int("product_id", idInt), zap.String("name", req.Name), zap.Error(err))
		return errors.ErrApiValidateUpdateProduct(c)
	}

	h.logger.Info("Attempting to update product", zap.String("handler", "Update"), zap.Int("product_id", idInt), zap.String("name", req.Name))
	...
	if err != nil {
		h.logger.Error("Failed to update product via command client", zap.Int("product_id", idInt), zap.Error(err))
		return errors.ErrApiProductFailedUpdate(c)
	}

	so := h.mapping.ToApiResponseProduct(res)
	h.logger.Info("Successfully updated product", zap.String("handler", "Update"), zap.Int("product_id", idInt))
...
func (h *productHandleApi) TrashedProduct(c echo.Context) error {
	...
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		h.logger.Warn("Invalid product ID provided in TrashedProduct", zap.String("handler", "TrashedProduct"), zap.String("param_id", c.Param("id")))
		return errors.ErrApiProductInvalidId(c)
	}

	h.logger.Info("Attempting to trash product", zap.String("handler", "TrashedProduct"), zap.Int("product_id", id))

	req := &pb.FindByIdProductRequest{Id: int32(id)}
	res, err := h.commandClient.Trashed(ctx, req)
	if err != nil {
		h.logger.Error("Failed to trash product via command client", zap.Int("product_id", id), zap.Error(err))
		return errors.ErrApiProductFailedTrashed(c)
	}

	so := h.mapping.ToApiResponsesProductDeleteAt(res)
	h.logger.Info("Successfully trashed product", zap.String("handler", "TrashedProduct"), zap.Int("product_id", id))
...
func (h *productHandleApi) RestoreProduct(c echo.Context) error {
	...
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		h.logger.Warn("Invalid product ID provided in RestoreProduct", zap.String("handler", "RestoreProduct"), zap.String("param_id", c.Param("id")))
		return errors.ErrApiProductInvalidId(c)
	}

	h.logger.Info("Attempting to restore product", zap.String("handler", "RestoreProduct"), zap.Int("product_id", id))

	req := &pb.FindByIdProductRequest{Id: int32(id)}
	res, err := h.commandClient.Restore(ctx, req)
	if err != nil {
		h.logger.Error("Failed to restore product via command client", zap.Int("product_id", id), zap.Error(err))
		return errors.ErrApiProductFailedRestore(c)
	}

	so := h.mapping.ToApiResponsesProductDeleteAt(res)
	h.logger.Info("Successfully restored product", zap.String("handler", "RestoreProduct"), zap.Int("product_id", id))
...
func (h *productHandleApi) DeleteProductPermanent(c echo.Context) error {
	...
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		h.logger.Warn("Invalid product ID provided in DeleteProductPermanent", zap.String("handler", "DeleteProductPermanent"), zap.String("param_id", c.Param("id")))
		return errors.ErrApiProductInvalidId(c)
	}

	h.logger.Info("Attempting to permanently delete product", zap.String("handler", "DeleteProductPermanent"), zap.Int("product_id", id))

	req := &pb.FindByIdProductRequest{Id: int32(id)}
	res, err := h.commandClient.DeleteProductPermanent(ctx, req)
	if err != nil {
		h.logger.Error("Failed to permanently delete product via command client", zap.Int("product_id", id), zap.Error(err))
		return errors.ErrApiProductFailedDeletePermanent(c)
	}

	so := h.mapping.ToApiResponseProductDelete(res)
	h.logger.Info("Successfully permanently deleted product", zap.String("handler", "DeleteProductPermanent"), zap.Int("product_id", id))
...
func (h *productHandleApi) RestoreAllProduct(c echo.Context) error {
	...
	h.logger.Info("Attempting to restore all trashed products", zap.String("handler", "RestoreAllProduct"))

	res, err := h.commandClient.RestoreAllProduct(ctx, &emptypb.Empty{})
	if err != nil {
		h.logger.Error("Failed to restore all products via command client", zap.Error(err))
		return errors.ErrApiProductFailedRestoreAll(c)
	}

	so := h.mapping.ToApiResponseProductAll(res)
	h.logger.Info("Successfully restored all trashed products", zap.String("handler", "RestoreAllProduct"))
...
func (h *productHandleApi) DeleteAllProductPermanent(c echo.Context) error {
	...
	h.logger.Info("Attempting to permanently delete all trashed products", zap.String("handler", "DeleteAllProductPermanent"))

	res, err := h.commandClient.DeleteAllProduct(ctx, &emptypb.Empty{})
	if err != nil {
		h.logger.Error("Failed to permanently delete all products via command client", zap.Error(err))
		return errors.ErrApiProductFailedDeleteAllPermanent(c)
	}

	so := h.mapping.ToApiResponseProductAll(res)
	h.logger.Info("Successfully permanently deleted all trashed products", zap.String("handler", "DeleteAllProductPermanent"))
